...
test('acidente: 3.000 combinações respeitam as invariantes', () => {
  ...
  for (let i = 0; i < 3000; i += 1) {
    ...
    const dados = {
      ...
      lado1: escolher(['direito', 'esquerdo']), lado2: escolher(['direito', 'esquerdo']),
      lado3: escolher(['direito', 'esquerdo']),
      concausa: aleatorio() < 0.3, contribuicaoTrabalho: escolher([0, 0, 40]), reducaoConcausa: escolher([50, 10, 90]),
